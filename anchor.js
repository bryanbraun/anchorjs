...
function addAnchors(selector, unWandedAnchors) {
  'use strict';
  ...
  var nbUnwanted = 0;
  if (unWandedAnchors) {
    if (typeof unWandedAnchors !== 'string') {
      throw new Error('AnchorJS accepts only strings for unwanted anchors; you used a ' + typeof unWandedAnchors);
    }
    unwanted = document.querySelectorAll(unWandedAnchors);
    nbUnwanted = unwanted.length;
  }
  ...
  for (var i = 0; i < elements.length; i++) {
    ...
    if (nbUnwanted > 0) {
      // Loop through the unwanted elements
      for (var j = 0; j < nbUnwanted; j++) {
        if (unwanted[j] === elements[i]) {
          isUnwanted = true;
          break;
        }
      }
